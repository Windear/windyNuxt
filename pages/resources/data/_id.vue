<template>
  <div class="detailsBody">
    <div class="detailsHeader">
      <div class="detailsTop">
        ...
        <div class="btn-box">
          ...
          <a
            class="baiducloud"
            :href="this.ip + '/media/' + projectData.resourcesDownload"
            @click="getResourcesDownloads()"
          ></a>
        </div>
      </div>
    </div>
    ...
    <el-dialog
      ...
      width="40%"
      custom-class="cloud_dialog"
      v-if="cloudDown!==0"
    >
      <div class="cloud_down" v-for="(item,key) in cloudDown" :key="item.id">
        <div class="baiduyun" v-if="item.drive_type==1">
          <img src="~/static/img/cloud.png" width="20px" />
          <span>百度云盘</span>
        </div>
        <div class="baiduyun" v-if="item.drive_type==2">
          <img src="~/static/img/weiyun.jpg" width="20px" />
          <span>
            腾讯微云
            <span style="color:#FF4949;">(推荐)</span>
          </span>
        </div>
        <p v-if="item.drive_pw">
          提取码"{{item.drive_pw}}"
          <a
            href="javascript:;"
            class="clipboard-btn"
            v-clipboard="item.drive_pw"
            @success="onCopy(key)"
            @error="onError(key)"
          >
            <span v-if="clipboardVal!=key&&clipboardVal!='err'">点击复制</span>
            <span class="clipboardVal-success" v-if="clipboardVal==key">复制成功</span>
            <span v-if="clipboardVal=='err'">复制失败，请自行复制</span>
          </a>
        </p>
...
export default {
  ...
  methods: {
    ...
    getResourcesData() {
      ...
      this.$store
        .dispatch("getResourcesData", params)
        .then(response => {
          ...
          if (response.statusText === "OK" && response.status === 200) {
            ...
            if (res.resourcesDownload.split("/")[0] != "resourceFile") {
              this.content = false;
            }
    ...
    getResourcesCloud() {
      ...
      //let params = localStorage.getItem("projectId");
      this.$store
        .dispatch("getResourcesCloud", params)
        .then(response => {
          let res = response.data;
          if (res.state != "err") {
            //console.log(res);
...
.payBox-leave-active {
  transition: all 0.5s cubic-bezier(1, 2, 0.8, 1);
  animation: reverse;
}
